...
class Magnet extends PassiveItem {
    ...
    constructor() {
        ...
        this.baseMagnetizeInterval = 15; // 15 seconds base
        this.magnetizeIntervalReductionPerLevel = 1.5; // Reduce by 1.5s per level
    ...
    calculateBonuses() {
        ...
        // Magnet interval is not a typical 'stat' for getStat, calculate directly
        this.magnetizeInterval = Math.max(5, this.baseMagnetizeInterval - (this.level -1) * this.magnetizeIntervalReductionPerLevel);
    }
    
    update(dt, owner) { // Added owner parameter for consistency if needed
        this.magnetizeTimer += dt;
        if (this.magnetizeTimer >= this.magnetizeInterval) {
            this.magnetizeTimer = 0;
            this.triggerMagnetEffect(owner); // Pass owner if effect needs it (e.g. player.x, player.y)
        }
    }

    triggerMagnetEffect(owner) { // Changed name, added owner
        if (!owner) return; // Guard clause
        const effect = {
            x: owner.x,
            y: owner.y,
            radius: 0,
            maxRadius: 300, // Or make this dynamic based on player's pickup radius
            lifetime: 0.5,
            timer: 0,
            isGarbage: false,
            update: function(dt_effect) { // Renamed dt to avoid conflict
                this.timer += dt_effect;
            ...
            draw: function(ctx_effect) { // Renamed ctx
                if (this.isGarbage) return;
                const alpha = 0.5 - (this.timer / this.lifetime) * 0.5;
                const screenPos = cameraManager.worldToScreen(this.x, this.y);
                ctx_effect.strokeStyle = `rgba(0, 100, 255, ${alpha})`;
                ctx_effect.lineWidth = 3;
                ctx_effect.beginPath();
                ctx_effect.arc(screenPos.x, screenPos.y, this.radius, 0, Math.PI * 2);
                ctx_effect.stroke();
            }
        };
        ...
        xpGems.forEach(gem => {
            if (!gem.isGarbage && gem.isActive) {
                gem.x = owner.x; // Use owner's position
                gem.y = owner.y;
            }
        });
    }
    ...
    getInitialDescription() {
        return `提升 ${this.basePickupRadiusBonus*100}% 拾取范围。每隔 ${this.magnetizeInterval.toFixed(1)} 秒自动吸取所有经验。 (当前 Lv1)`;
    }
    ...
    getUpgradeDescription() {
        ...
        const nextLevelInterval = Math.max(5, this.baseMagnetizeInterval - this.level * this.magnetizeIntervalReductionPerLevel);
        return `再提升 ${this.basePickupRadiusBonus*100}% 拾取范围。吸取间隔减少至 ${nextLevelInterval.toFixed(1)}秒 (Lv${this.level + 1} 总计范围: +${((this.level + 1) * this.basePickupRadiusBonus * 100).toFixed(0)}%)。`;
    }

    // Removed applyInitialBonus and onUpgrade as calculateBonuses handles the interval now.
}
